...
const playerLobbyMap = new Map(); // <ws, lobby_id>
...
function broadcastChat(lobby_id, player_name, message) {
    ...
    if (!lobby) return;
    const chatMsg = JSON.stringify({ type: "chat_message", player_name, message });
    lobby.players.forEach(p => { if (p.ws.readyState === 1) p.ws.send(chatMsg); });
}
...
function broadcastPlayers(lobby_id) {
    ...
    const data = JSON.stringify({ type: "players_online", players: playerNames });
    lobby.players.forEach(p => { if (p.ws.readyState === 1) p.ws.send(data); });
}
        ...
        try {
            ...
            switch (data.type) {
                ...
                case "create_lobby":
                    ...
                    playerLobbyMap.set(ws, newLobby.id);
                ...
                case "join_lobby":
                    ...
                    playerLobbyMap.set(ws, lobby.id);
                ...
                case "chat_message":
                    const lid = playerLobbyMap.get(ws);
                    if (!lid) return; // il giocatore non appartiene a nessuna lobby
                    broadcastChat(lid, data.player_name, data.message);
                    break;
    ...
    ws.on('close', () => {
        ...
        if (lid) {
            ...
            if (lobby) {
                ...
                if (idx !== -1) {
                    console.log(`🔴 ${lobby.players[idx].name} ha lasciato ${lobby.name}`);
